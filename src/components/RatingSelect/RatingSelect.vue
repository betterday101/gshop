<template>
  <div class="ratingselect">
    <div class="rating-type border-1px">
      <span class="block" :class="{active:type===2}" @click="setSelectType(2)">
        全部<span class="count">{{totalRatings}}</span>
      </span>
      <span class="block" :class="{active:type===0}" @click="setSelectType(0)">
        推荐<span class="count">{{totalPositiveRatings}}</span>
      </span>
      <span class="block" :class="{active:type===1}" @click="setSelectType(1)">
    ...
    export default {
        ...
        methods: {
          setSelectType (selectType) {
            // 触发处理事件
            this.$emit('selectType', selectType)
          }
        }
    }
</script>
